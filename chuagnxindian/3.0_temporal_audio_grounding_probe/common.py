"""Shared zero-training and evaluator helpers for Experiment 3.0."""
...
def fuse_normalized(first: np.ndarray, second: np.ndarray, alpha: float = 0.6) -> np.ndarray:
    return normalize_map(alpha * first + (1.0 - alpha) * second)


def resize_maps(tensors: dict[str, torch.Tensor], mode: str = "bicubic") -> dict[str, np.ndarray]:
    return {
        name: F.interpolate(tensor, (224, 224), mode=mode, align_corners=False)
        .detach()
        .cpu()
        .numpy()[:, 0]
...
def source_artifact_paths(registry: dict[str, Any]) -> dict[str, Path]:
    return {
        "experiment_2_2_reference_summary": R22_ROOT / registry["reference_key"] / "summary.json",
    }
