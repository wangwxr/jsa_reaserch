...
output_root="${SLOT_PROBE_OUTPUT_ROOT:-${script_dir}/results}"
...
echo "Experiment 2.0 is evaluation-only: no optimizer, backward, or checkpoint writes."
...
if (( vgg_status != 0 || flickr_status != 0 )); then
  echo "Probe failed: VGG status=${vgg_status}, Flickr status=${flickr_status}" >&2
  exit 1
fi
