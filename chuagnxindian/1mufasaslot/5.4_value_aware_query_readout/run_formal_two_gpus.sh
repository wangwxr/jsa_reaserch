...
output_root="${VALUE_QUERY_OUTPUT_ROOT:-${script_dir}/results}"
...
echo "Experiment 5.4: zero-training ATT_FINE @ V34 value-aware query readout"
...
if (( vgg_status != 0 || flickr_status != 0 )); then
  echo "Experiment 5.4 failed: VGG=${vgg_status}, Flickr=${flickr_status}" >&2
  exit 1
fi
