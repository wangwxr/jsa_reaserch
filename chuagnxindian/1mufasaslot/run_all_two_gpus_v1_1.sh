...
run_queue() {
    ...
    for split in 10k 144k; do
        if [ "$dataset" = "vggss" ]; then
            experiment_name="mufasa_jsa_v1_1_vggss_${split}"
        else
            experiment_name="mufasa_jsa_v1_1_flickr_${split}_frame8_center5"
        fi
        echo "Starting ${experiment_name} on GPU ${gpu}"
        bash "$script_root/train_${dataset}_mufasa_v1_1.sh" \
...
echo "All four MUFASA-JSA v1.1 experiments completed successfully."
