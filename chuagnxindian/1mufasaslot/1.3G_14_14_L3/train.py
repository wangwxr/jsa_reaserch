#!/usr/bin/env python3
"""1.3G_14_14_L3: unchanged G trained from a native-L3 semantic teacher."""
...
def run_model_sanity(
    ...
    expected_aud = teacher_reference_metrics(registry)
    if not metric_matches(baseline_metrics["AUD_L4"], expected_aud):
    ...
    checks = {
        **geometry_checks,
        "teacher_source": str((PROJECT_ROOT / "checkpoints" / registry["base_experiment"] / registry["base_checkpoint"]).resolve()),
        "teacher_architecture": "1.1.1_14_14_L3",
        "expected_teacher_AUD_cIoU_AUC": list(expected_aud),
        ...
        "F3_spatial_shape": list(output["F3_SPATIAL_A"].shape),
        "F4_shape": list(output["F4_A"].shape),
        "F4_up_shape": list(output["F4_UP_A"].shape),
        "F34_shape": list(output["F34_A"].shape),
        "fine_tokens_shape": list(output["FINE_TOKENS_A"].shape),
        "fine_keys_shape": list(output["FINE_KEYS_A"].shape),
        "Qa_shape": list(output["QA"].shape),
        "AUD_FINE_shape": list(output["AUD_FINE_A"].shape),
    }
    ...
    expected_shapes = {
        "F3_spatial_shape": [frame.shape[0], 512, 14, 14],
        "F4_shape": [frame.shape[0], 512, 7, 7],
        "F4_up_shape": [frame.shape[0], 512, 14, 14],
        "F34_shape": [frame.shape[0], 512, 14, 14],
        "fine_tokens_shape": [frame.shape[0], 196, 512],
        "fine_keys_shape": [frame.shape[0], 196, 512],
        "Qa_shape": [frame.shape[0], 2, 512],
        "AUD_FINE_shape": [frame.shape[0], 1, 14, 14],
    }
    for key, expected in expected_shapes.items():
        if checks[key] != expected:
            raise RuntimeError(f"G shape audit failed for {key}: {checks[key]} != {expected}")
...
def main() -> None:
    ...
    audit_dir = (
        arguments.model_dir / "1.3G_14_14_L3_sanity" / arguments.experiment
