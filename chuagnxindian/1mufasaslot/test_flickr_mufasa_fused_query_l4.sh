...
log_file="$log_root/${experiment_name}_test_flickr_fused_query_l4_${checkpoint_label}_${timestamp}.log"
exec > >(tee -a "$log_file") 2>&1
echo "Test log: $log_file"
echo "Architecture: mufasa_jsa_v1; eval: fused_query_l4; experiment: $experiment_name; checkpoint: $checkpoint; GPU: $gpu"

cd "$project_root"
MUFASA_EVAL_ATTENTION_MODE=fused_query_l4 \
    "$python_bin" "$experiment_root/test_mufasa_l4.py" \
