...
log_dir="$project_root/checkpoints/1.5G-v3semantic_priority_gradient_refine_sanity/logs"
mkdir -p "$log_dir"
...
echo "Experiment G-v3 sanity: VGG PID=$vgg_pid GPU=0; Flickr PID=$flickr_pid GPU=1"
status=0
wait "$vgg_pid" || status=$?
wait "$flickr_pid" || status=$?
if [ "$status" -eq 0 ]; then
    echo "Both Experiment G-v3 sanity checks passed."
fi
exit "$status"
