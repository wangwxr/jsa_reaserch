...
log_dir="$project_root/checkpoints/1.3G-multigeom_equivariant_l3_refine_sanity/logs"
mkdir -p "$log_dir"
...
echo "Experiment G sanity: VGG PID=$vgg_pid GPU=0; Flickr PID=$flickr_pid GPU=1"
vgg_status=0
flickr_status=0
wait "$vgg_pid" || vgg_status=$?
wait "$flickr_pid" || flickr_status=$?
if [ "$vgg_status" -ne 0 ] || [ "$flickr_status" -ne 0 ]; then
    echo "Sanity failed: VGG status=$vgg_status, Flickr status=$flickr_status"
    exit 1
fi
echo "Both Experiment G sanity checks passed."
