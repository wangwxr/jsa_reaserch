...
experiment_name=${2:-1.3G-v2_vggss_10k}
...
echo "1.3G-v2 VGGSoundSS-10k: GPU=$gpu, epochs=100"
cd "$project_root"
"$python_bin" "$script_dir/train.py" --experiment vggss_10k --gpu "$gpu" \
    --epochs 100 --experiment-name "$experiment_name"
"$python_bin" "$script_dir/evaluate_full.py" --experiment vggss_10k --gpu "$gpu" \
