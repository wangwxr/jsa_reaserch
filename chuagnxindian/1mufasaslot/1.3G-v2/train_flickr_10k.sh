...
experiment_name=${2:-1.3G-v2_flickr_10k_frame8_center5}
...
echo "1.3G-v2 Flickr-10k: GPU=$gpu, epochs=100"
cd "$project_root"
"$python_bin" "$script_dir/train.py" --experiment flickr_10k --gpu "$gpu" \
    --epochs 100 --experiment-name "$experiment_name"
"$python_bin" "$script_dir/evaluate_full.py" --experiment flickr_10k --gpu "$gpu" \
