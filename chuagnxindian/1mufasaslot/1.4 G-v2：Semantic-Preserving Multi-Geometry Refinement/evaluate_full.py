#!/usr/bin/env python3
"""Evaluate a G-v2 best checkpoint with the standard six JSA maps."""
...
def main() -> None:
    ...
    if checkpoint.get("architecture") != "semantic_preserving_multi_geometry_refine_g_v2":
    ...
    print(f"Experiment G-v2 checkpoint: {checkpoint_path}", flush=True)
    ...
    result = {
        ...
        "metrics": metrics,
        "delta_ogl": {
            "cIoU": metrics["OGL"]["cIoU"] - metrics["AUD"]["cIoU"],
            "AUC": metrics["OGL"]["AUC"] - metrics["AUD"]["AUC"],
        },
    }
    ...
    print(
        "DELTA_OGL/cIoU,AUC "
        f"{result['delta_ogl']['cIoU']:.4f} {result['delta_ogl']['AUC']:.4f}",
        flush=True,
    )
