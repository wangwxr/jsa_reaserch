...
def gradient_diagnostic(
    ...
    """Read coarse/equiv gradient geometry without touching parameter .grad."""
    # The formal backward uses GradScaler. Mirror its numerical protection for
    # these read-only autograd.grad calls so FP16 intermediate gradients do not
    # underflow, then divide the returned gradients back to their true scale.
    diagnostic_scale = 65536.0
    coarse = loss_gradients(loss_coarse * diagnostic_scale, parameters)
    equiv = loss_gradients(loss_equiv * diagnostic_scale, parameters)
    device = loss_coarse.device
    coarse_sq = torch.zeros((), device=device, dtype=torch.float32)
    equiv_sq = torch.zeros((), device=device, dtype=torch.float32)
    dot = torch.zeros((), device=device, dtype=torch.float32)
    for coarse_grad, equiv_grad in zip(coarse, equiv):
        if coarse_grad is not None:
            coarse_grad = coarse_grad.float() / diagnostic_scale
            coarse_sq = coarse_sq + coarse_grad.square().sum()
        if equiv_grad is not None:
            equiv_grad = equiv_grad.float() / diagnostic_scale
            equiv_sq = equiv_sq + equiv_grad.square().sum()
        if coarse_grad is not None and equiv_grad is not None:
            dot = dot + (coarse_grad * equiv_grad).sum()
    norm_coarse = coarse_sq.sqrt()
    norm_equiv = equiv_sq.sqrt()
    eps = 1e-12
    cosine = dot / (norm_coarse * norm_equiv + eps)
    return {
        "grad_norm_coarse": float(norm_coarse.detach()),
        "grad_norm_equiv": float(norm_equiv.detach()),
        "grad_norm_ratio": float((norm_equiv / (norm_coarse + eps)).detach()),
...
def run_model_sanity(
    ...
    print("Running Experiment G-v2 geometry and model sanity checks...", flush=True)
    ...
    expected_total_gradients = loss_gradients(
        losses["loss_total"], trainable_parameters
    )
    grad_was_clear_before = all(
        parameter.grad is None for parameter in trainable_parameters
    )
    diagnostic = gradient_diagnostic(
        losses["loss_coarse"], losses["loss_equiv"], trainable_parameters
    )
    grad_clear_after_diagnostic = all(
        parameter.grad is None for parameter in trainable_parameters
    )
    losses["loss_total"].backward()
    diagnostic_backward_max_error = 0.0
    for expected, parameter in zip(expected_total_gradients, trainable_parameters):
        if expected is None and parameter.grad is None:
            continue
        if expected is None or parameter.grad is None:
            diagnostic_backward_max_error = math.inf
            break
        diagnostic_backward_max_error = max(
            diagnostic_backward_max_error,
            float((expected.detach() - parameter.grad.detach()).abs().max()),
        )
    ...
    checks = {
        **geometry_checks,
        ...
        "gradient_diagnostic_grad_clear_before": grad_was_clear_before,
        "gradient_diagnostic_grad_clear_after": grad_clear_after_diagnostic,
        "gradient_diagnostic_backward_max_abs_error": diagnostic_backward_max_error,
        **diagnostic,
    ...
    if (
        not grad_was_clear_before
        or not grad_clear_after_diagnostic
        or diagnostic_backward_max_error > 1e-6
    ):
        raise RuntimeError("Gradient diagnostic changed normal optimizer gradients")
    ...
    print("All Experiment G-v2 sanity checks passed.", flush=True)
...
def train_one_epoch(
    ...
    diagnostic_totals = {field: 0.0 for field in diagnostic_fields}
    diagnostic_count = 0
    negative_cosine_count = 0
    last_diagnostic = {field: math.nan for field in diagnostic_fields}
    ...
    for batch_index, (frame, spec, _bboxes, _names, _labels) in enumerate(train_loader):
        ...
        if batch_index % arguments.gradient_diagnostic_interval == 0:
            ...
            if any(parameter.grad is not None for parameter in trainable_parameters):
                raise RuntimeError("Gradient diagnostic started with non-empty .grad")
            last_diagnostic = gradient_diagnostic(
                losses["loss_coarse"], losses["loss_equiv"], trainable_parameters
            )
            if any(parameter.grad is not None for parameter in trainable_parameters):
                raise RuntimeError("Gradient diagnostic modified parameter .grad")
            diagnostic_count += 1
            for field in diagnostic_fields:
                diagnostic_totals[field] += last_diagnostic[field]
            negative_cosine_count += int(last_diagnostic["grad_cosine"] < 0)
        ...
        if batch_index % 10 == 0 or completed == len(train_loader):
            ...
            print(
                ...
                f"g_ratio={last_diagnostic['grad_norm_ratio']:.3f} "
                f"g_cos={last_diagnostic['grad_cosine']:.3f} ETA={eta}",
                flush=True,
            )

    if diagnostic_count == 0:
        raise RuntimeError("No gradient diagnostic was recorded")
    ...
    result.update(
        {
            ...
            **{
                field: diagnostic_totals[field] / diagnostic_count
                for field in diagnostic_fields
            },
            "negative_grad_cosine_ratio": negative_cosine_count / diagnostic_count,
...
def save_checkpoint(
    ...
    checkpoint = {
        "architecture": "semantic_preserving_multi_geometry_refine_g_v2",
...
def main() -> None:
    ...
    if arguments.lambda_equiv != 1.0:
        raise ValueError("G-v2 fixes lambda_equiv=1.0")
    if arguments.init_lr != 5e-5 or arguments.weight_decay != 0.01:
        raise ValueError("G-v2 fixes AdamW lr=5e-5 and weight_decay=0.01")
    if arguments.epochs != 10:
        raise ValueError("The first G-v2 experiment is fixed to 10 epochs")
    if (
        ...
        raise ValueError("G-v2 crop, flip, teacher-mass, and valid-ratio settings are fixed")
    ...
    audit_dir = (
        arguments.model_dir / "1.4G-v2-semantic_preserving_multigeom_sanity" / arguments.experiment
    ...
    for epoch in range(epochs):
        ...
        print(
            ...
            f"grad_coarse={train_metrics['grad_norm_coarse']:.6f} "
            f"grad_equiv={train_metrics['grad_norm_equiv']:.6f} "
            f"grad_ratio={train_metrics['grad_norm_ratio']:.4f} "
            f"grad_cos={train_metrics['grad_cosine']:.4f} "
            f"neg_cos={train_metrics['negative_grad_cosine_ratio']:.4f}",
            flush=True,
        )
    ...
    print(
        f"Total Experiment G-v2 training time: {timedelta(seconds=int(time.time() - run_start))}",
        flush=True,
    )
    model.close()
