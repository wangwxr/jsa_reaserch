"""Data, checkpoint, and model construction for Experiment F."""
...
from model import FrozenTeacherEquivariantRefinement  # noqa: E402
...
EXPERIMENTS = {
    "vggss_10k": {
        ...
        "default_experiment": "equivariant_l3_refine_vggss_10k",
        "eval_batch_size": 256,
        "workers": 16,
    ...
    "flickr_10k": {
        ...
        "default_experiment": "equivariant_l3_refine_flickr_10k_frame8_center5",
        "eval_batch_size": 32,
        "workers": 12,
...
def build_model(
...
) -> tuple[FrozenTeacherEquivariantRefinement, Path]:
    ...
    model = FrozenTeacherEquivariantRefinement(teacher).to(device)
...
def parameter_counts(model: FrozenTeacherEquivariantRefinement) -> dict[str, int]:
    ...
    adapter_total = sum(
        parameter.numel() for parameter in model.student.adapter.parameters()
    )
    trainable = sum(
