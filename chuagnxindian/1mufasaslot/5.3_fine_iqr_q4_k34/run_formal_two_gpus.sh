...
output_root="${FINE_IQR_OUTPUT_ROOT:-${script_dir}/results}"
...
echo "Experiment 5.3: zero-training Q4 -> K34 Fine IQR diagnostic"
...
if (( vgg_status != 0 || flickr_status != 0 )); then
  echo "Experiment 5.3 failed: VGG=${vgg_status}, Flickr=${flickr_status}" >&2
  exit 1
fi
