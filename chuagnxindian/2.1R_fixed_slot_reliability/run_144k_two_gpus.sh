...
output_root="${FIXED_RELIABILITY_OUTPUT_ROOT:-${script_dir}/results}"
...
echo "Experiment 2.1R: zero-training fixed-slot reliability."
...
if (( vgg_status != 0 || flickr_status != 0 )); then
  echo "2.1R failed: VGG=${vgg_status}, Flickr=${flickr_status}" >&2
  exit 1
fi
