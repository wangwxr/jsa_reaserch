"""Shared zero-training utilities for Experiment 4.0."""
...
def verify_snapshots(before: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for role, reference in before.items():
        ...
        same = after_hash == reference["sha256"] and after_mtime == reference["mtime_ns"]
        files[role] = {
            **reference,
    ...
    return {"files": files, "all_unchanged": unchanged}
...
def stage1_checkpoint_path(registry: dict[str, Any]) -> Path:
    return PROJECT_ROOT / "checkpoints" / registry["stage1"] / registry["checkpoint"]


def g_checkpoint_path(registry: dict[str, Any]) -> Path:
    return PROJECT_ROOT / "checkpoints" / registry["g"] / registry["checkpoint"]


def load_stage1_path(
    registry: dict[str, Any], checkpoint_path: Path, device: torch.device
) -> MUFASAL3L4:
...
def load_stage1(registry: dict[str, Any], device: torch.device) -> MUFASAL3L4:
    return load_stage1_path(registry, stage1_checkpoint_path(registry), device)
...
def load_original_g(registry: dict[str, Any], device: torch.device) -> GRefinement:
    ...
    checkpoint = torch.load(g_checkpoint_path(registry), map_location="cpu", weights_only=False)
...
def normalize_map(value: np.ndarray) -> np.ndarray:
    # Match root utils.normalize_img exactly: keep the evaluator input dtype.
    # Promoting float32 maps before fusion can move threshold-boundary samples.
...
def resize_tensor(value: torch.Tensor) -> torch.Tensor:
    return F.interpolate(value, (224, 224), mode="bicubic", align_corners=False)
...
def distribution(values: list[float]) -> dict[str, float | int]:
    ...
    if finite.size == 0:
        return {key: math.nan for key in ("mean", "median", "std", "p10", "p25", "p75", "p90")} | {"num_samples": 0}
    return {
...
def read_epoch_rows(registry: dict[str, Any]) -> list[dict[str, str]]:
    path = PROJECT_ROOT / "checkpoints" / registry["stage1"] / "epoch_metrics.csv"
    with path.open(newline="", encoding="utf-8") as handle:
        return list(csv.DictReader(handle))


def stage1_epoch_reference(registry: dict[str, Any]) -> dict[str, dict[str, float]]:
    checkpoint = torch.load(
        stage1_checkpoint_path(registry), map_location="cpu", weights_only=False
    )
    epoch = int(checkpoint["epoch"])
    row = next(value for value in read_epoch_rows(registry) if int(value["epoch"]) == epoch)
    mapping = {
        "AUD": ("aud_ciou", "aud_auc"),
        "IMG": ("img_query_ciou", "img_query_auc"),
        "IQR": ("iqr_ciou", "iqr_auc"),
        "OBJ": ("obj_prior_ciou", "obj_prior_auc"),
        "OGL": ("ogl_ciou", "ogl_auc"),
    }
    return {
        name: {"cIoU": float(row[ciou]), "AUC": float(row[auc])}
        for name, (ciou, auc) in mapping.items()
    }


def stage1_reference(registry: dict[str, Any]) -> dict[str, dict[str, float]]:
    """Read the independent best-checkpoint test log, not the training epoch row."""
    log_root = PROJECT_ROOT / "checkpoints" / registry["stage1"] / "logs"
    logs = sorted(log_root.glob(f"*test*{registry['checkpoint'].removesuffix('.pth')}*.log"))
    if not logs:
        raise FileNotFoundError(f"No standalone test log under {log_root}")
    text = logs[-1].read_text(encoding="utf-8")
    prefixes = {"AUD": "AUD", "IMG": "IMG_QUERY", "IQR": "IQR", "OBJ": "OBJ_PRIOR", "OGL": "OGL"}
    output = {}
    for name, prefix in prefixes.items():
        match = re.search(
            rf"^{prefix}_{registry['dataset']}/cIoU, auc\s+([0-9.]+)\s+([0-9.]+)$",
            text,
            flags=re.MULTILINE,
        )
        if match is None:
            raise RuntimeError(f"Missing {prefix} in {logs[-1]}")
        output[name] = {"cIoU": float(match.group(1)), "AUC": float(match.group(2))}
    return output


def stage2_reference(registry: dict[str, Any]) -> dict[str, dict[str, float]]:
    path = PROJECT_ROOT / "checkpoints" / registry["g"] / "best_full_six_metrics.json"
    payload = json.loads(path.read_text(encoding="utf-8"))["metrics"]
    return {
        "AUD": payload["AUD"],
        "IMG": payload["IMG_QUERY"],
        "IQR": payload["IQR"],
        "OBJ": payload["OBJ_PRIOR"],
        "OGL": payload["OGL"],
    }
