...
source_root=${VGGSS_FLAT_ROOT:-$project_root/prepared/jsa_vggss_144k_clean}
target_root=${VGGSS_NVME_ROOT:-/home/wxr/datasets/JSA/VGGSound_144k_npy}
manifest=${VGGSS_MANIFEST:-$project_root/metadata_vggss/vggss_144k.csv}
python_bin=${JSA_PYTHON:-/home/wxr/miniconda3/envs/wwww/bin/python}
log_experiment=${JSA_LOG_EXPERIMENT:-jsa_vggss_144k}

log_root=${JSA_LOG_ROOT:-$project_root/checkpoints/$log_experiment/logs}
mkdir -p "$log_root"
timestamp=$(date +%Y%m%d_%H%M%S)
log_file="$log_root/vggss_nvme_prepare_${timestamp}.log"
